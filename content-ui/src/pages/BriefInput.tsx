...
export default function BriefInput() {
  ...
  const handleSubmit = async () => {
    if (!brief.trim()) {
      setError("Please enter a content brief.");
      return;
    }
    if (!companyId) {
      setError("No company loaded. Go to Onboarding first.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    try {
      ...
      setError(
        "Failed to reach backend. Make sure uvicorn is running on port 8001.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      >
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p
            style={{ color: D.sub, fontSize: 13.5, margin: 0, paddingLeft: 46 }}
          >
            {companyName ? (
              <>
                Creating for{" "}
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Describe what you want to create. E.g. — Write a LinkedIn post announcing Payzen Magic Checkout and its impact on reducing cart abandonment for Indian e-commerce merchants."
              value={brief}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {CONTENT_TYPES.map((t) => (
                <button
                  key={t}
                  className="chip-btn"
                  style={chip(contentType === t, D.purple)}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {LANGUAGES.map((l) => (
                <button
                  key={l}
                  className="chip-btn"
                  style={chip(languages.includes(l), D.cyan)}
          ...
          >
            {[
              ...
              { label: "Type", value: contentType },
              {
                label: "Languages",
                value:
                  languages.map((l) => LANGUAGE_LABELS[l]).join(", ") || "—",
              },
