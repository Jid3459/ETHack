...
import { BrandViolation, LegalFlag, SEOSuggestion } from '../types'
...
function ScoreRing({ score, size = 96 }: { score: number; size?: number }) {
  ...
  return (
    <motion.svg width={size} height={size} initial={{ rotate: -90 }} style={{ display:'block' }}>
      <defs>
        <filter id={glowId}>
      ...
      {/* track */}
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#1e2d45" strokeWidth="6"/>
      {/* filled */}
      <motion.circle
      ...
      <text x={size/2} y={size/2 + 1} textAnchor="middle" dominantBaseline="middle"
        ...
        fontFamily="-apple-system,sans-serif" transform={`rotate(90,${size/2},${size/2})`}>
        {score}
      </text>
      <text x={size/2} y={size/2 + size * 0.16} textAnchor="middle" dominantBaseline="middle"
        fill={T.sub} fontSize={size * 0.11} fontFamily="-apple-system,sans-serif"
...
// ── Draft Panel ─────────────────────────────────────────────────────────────────
function DraftPanel({ draft, brandViolations, legalFlags }: {
  draft: string; brandViolations: BrandViolation[]; legalFlags: LegalFlag[]
}) {
  const brandPhrases = brandViolations.map(v => v.phrase)
  const legalPhrases  = legalFlags.map(f => f.phrase)

  function highlightLine(line: string) {
    let parts: { text: string; type: 'brand'|'legal'|'normal' }[] = [{ text: line, type: 'normal' }]
    ...
    return parts.map((p, i) => {
      if (p.type === 'brand') return (
        <mark key={i} title="Brand violation" style={{
          background: 'rgba(245,158,11,0.2)', color: '#fbbf24',
          borderBottom: `2px solid ${T.amber}`, padding: '0 3px', borderRadius: 2,
          cursor: 'help', textDecoration: 'none',
        }}>{p.text}</mark>
      )
      if (p.type === 'legal') return (
        <mark key={i} title="Legal flag" style={{
          background: 'rgba(239,68,68,0.15)', color: '#f87171',
          borderBottom: `2px solid ${T.red}`, padding: '0 3px', borderRadius: 2,
          cursor: 'help', textDecoration: 'none',
        }}>{p.text}</mark>
      )
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ color: T.text, fontSize: 13, fontWeight: 600, letterSpacing: '0.02em' }}>
            Content Draft
          </span>
        </div>
        <div style={{ display: 'flex', gap: 14 }}>
          {[{ color: T.amber, label: 'Brand violation' }, { color: T.red, label: 'Legal flag' }].map(item => (
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: T.sub }}>
              <div style={{ width: 18, height: 3, backgroundColor: item.color, borderRadius: 2, opacity: 0.9 }} />
              {item.label}
            </div>
          ))}
        </div>
...
// ── Violation Card ──────────────────────────────────────────────────────────────
function ViolationCard({ children, level, delay = 0 }: {
  children: React.ReactNode; level: 'high'|'medium'|'low'; delay?: number
}) {
...
// ── Collapsible Section ─────────────────────────────────────────────────────────
function Section({ title, icon, count, color, children, defaultOpen = false }: {
...
// ── Brand Panel ─────────────────────────────────────────────────────────────────
function BrandPanel({ score, violations }: { score: number; violations: BrandViolation[] }) {
  const color = score >= 80 ? T.green : T.red
  return (
    <Section title="Brand Compliance" icon="⬡" count={violations.length} color={color} defaultOpen>
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 8 }}>
        <ScoreRing score={score} size={80} />
        <div>
          <div style={{ color: T.text, fontSize: 15, fontWeight: 700 }}>
            {score >= 80 ? 'Brand Approved' : 'Needs Revision'}
          </div>
          <div style={{ color: T.sub, fontSize: 12, marginTop: 4 }}>
            {violations.length} violation{violations.length !== 1 ? 's' : ''} detected
          </div>
          <div style={{
            marginTop: 8, width: 140, height: 4, borderRadius: 2,
      ...
      {violations.map((v, i) => (
        <ViolationCard key={i} level={v.severity} delay={i * 0.07}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Pill label={v.severity} color={sevColor(v.severity)} bg={sevBg(v.severity)} />
            <span style={{ fontFamily: T.mono, fontSize: 12, color: T.amber }}>"{v.phrase}"</span>
          </div>
          <div style={{ color: T.sub, fontSize: 12, marginBottom: 6 }}>{v.reason}</div>
          <div style={{ color: T.green, fontSize: 12, display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{ opacity: 0.7 }}>→</span> {v.suggestion}
          </div>
        </ViolationCard>
      ))}
    </Section>
...
function LegalPanel({ flags }: { flags: LegalFlag[] }) {
  return (
    <Section title="Legal Review" icon="⚖" count={flags.length} color={T.amber}>
      {flags.length === 0 && (
        <div style={{ color: T.green, fontSize: 13, padding: '8px 0' }}>✓ No legal flags</div>
      )}
      {flags.map((f, i) => (
        <ViolationCard key={i} level={f.risk_level} delay={i * 0.07}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Pill label={`${f.risk_level} risk`} color={sevColor(f.risk_level)} bg={sevBg(f.risk_level)} />
            <span style={{ fontFamily: T.mono, fontSize: 12, color: '#f87171' }}>"{f.phrase}"</span>
          </div>
          <div style={{
            fontFamily: T.mono, fontSize: 11, color: T.amber,
            background: '#0a0e1a', padding: '6px 10px', borderRadius: 6, marginBottom: 8,
          }}>
            {f.regulation} § {f.section}
          </div>
          <div style={{ color: T.sub, fontSize: 12, marginBottom: 5 }}>{f.plain_english}</div>
...
// ── SEO Panel ───────────────────────────────────────────────────────────────────
function SEOPanel({ suggestions }: { suggestions: SEOSuggestion[] }) {
  return (
    <Section title="SEO Advisory" icon="◎" count={suggestions.length} color={T.cyan}>
      {suggestions.length === 0 && (
        <div style={{ color: T.green, fontSize: 13, padding: '8px 0' }}>✓ SEO looks good</div>
      )}
      {suggestions.map((s, i) => (
        <motion.div key={i}
          ...
          <div style={{
            ...
            backgroundColor: T.cyan, marginTop: 5, flexShrink: 0,
            boxShadow: `0 0 6px ${T.cyan}`,
          }} />
          <div>
            <div style={{ color: T.text, fontSize: 13, marginBottom: 4 }}>{s.message}</div>
            <div style={{ color: T.sub, fontSize: 11 }}>
              Impact: <span style={{ color: sevColor(s.impact) }}>{s.impact}</span>
            </div>
          </div>
        </motion.div>
      ))}
...
// ── Decision Bar ────────────────────────────────────────────────────────────────
function DecisionBar({ decision, setDecision, feedback, setFeedback, submitting, onSubmit }: {
  ...
  return (
    ...
    >
      <div style={{ color: T.sub, fontSize: 10, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 14 }}>
        Your Decision
      </div>
      <div style={{ display: 'flex', gap: 12, marginBottom: decision === 'reject' ? 16 : 0 }}>
        {/* Approve */}
        <motion.button whileHover={{ scale: 1.015 }} whileTap={{ scale: 0.98 }}
          ...
          style={{
            ...
            transition: 'all 0.2s',
            boxShadow: decision === 'approve' ? `0 0 20px ${T.green}30` : 'none',
          }}>
      ...
      <AnimatePresence>
        {decision === 'reject' && (
          <motion.div
            ...
            <div style={{ color: T.sub, fontSize: 12, marginBottom: 8, marginTop: 4 }}>
            ...
            <textarea
              ...
              style={{
                width: '100%', height: 80,
                backgroundColor: '#080b14', border: `1px solid ${T.border}`,
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <motion.button
              ...
              style={{
                ...
                background: submitting ? T.dim : `linear-gradient(135deg, ${T.accent}, #6366f1)`,
...
export default function HumanApproval() {
  ...
  const { runId } = useApp()
  const approval = MOCK_APPROVAL_DATA
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (USE_MOCK) {
        mockApprove(runId || 'mock_demo_run', decision)
        ...
        <motion.div
          ...
          <motion.div
            ...
            style={{
              ...
              fontSize: 32, boxShadow: `0 0 30px ${ok ? T.green : T.red}30`,
            }}>
            {ok ? '✓' : '✗'}
          </motion.div>
          ...
          <div style={{ color: T.sub, fontSize: 14, lineHeight: 1.6 }}>
            {ok ? 'Resuming pipeline — localisation and distribution running…'
  ...
  // ── Header strip ─────────────────────────────────────────────────────────────
  const metricCards = [
    { label: 'Brand Score', value: `${approval.brand_score}/100`,
      color: approval.brand_score >= 80 ? T.green : T.red },
    { label: 'Legal Flags', value: String(approval.legal_flags.length),
      color: approval.legal_flags.length ? T.amber : T.green },
    { label: 'SEO Tips', value: `${approval.seo_suggestions?.length ?? 0}`,
      color: T.cyan },
  ]
        ...
        >
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <h1 style={{ color: T.text, fontSize: 24, fontWeight: 700, margin: 0, letterSpacing: '-0.02em' }}>
                Human Review Gate
              </h1>
            </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {metricCards.map(m => (
              <motion.div key={m.label}
                ...
                <div style={{ color: T.sub, fontSize: 10, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  {m.label}
                </div>
