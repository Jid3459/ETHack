...
export default function KnowledgeUploader({ companyId }: { companyId: string }) {
  ...
  const handleFile = async (file: File) => {
    ...
    if (!['pdf', 'docx', 'csv', 'txt', 'md'].includes(ext)) {
      setUpload({ status: 'error', error: `Unsupported format .${ext}. Use PDF, DOCX, CSV, TXT, or MD.` })
      return
    }
    ...
    try {
      const r = await fetch(`${BASE}/ingest/${companyId}`, { method: 'POST', body: fd })
      if (!r.ok) {
        const err = await r.json()
        throw new Error(err.detail || 'Upload failed')
      }
      const data = await r.json()
      setUpload({ status: 'success', fileName: file.name, chunks: data.chunks_stored })
      await loadDocs()
      // Auto-reset after 4s
    ...
    <div style={{ marginBottom: 4 }}>
      ...
      >
        <div style={{
          ...
          {docCount > 0 && (
            <span style={{
              ...
              fontSize: 10.5, color: '#a78bfa', fontWeight: 700,
            }}>
        ...
        <motion.span
          ...
          style={{ color: D.dim, fontSize: 10, display: 'inline-block', flexShrink: 0 }}
        >▾</motion.span>
      </button>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div style={{
              background: D.card, border: `1px solid rgba(139,92,246,0.2)`,
              borderRadius: 12, padding: '16px', marginBottom: 6,
              ...
              >
                ...
                {upload.status === 'idle' && (
                  <>
                    ...
                    <div style={{ color: '#a78bfa', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
                      {dragging ? 'Drop to upload' : 'Drop a document or click to browse'}
                    </div>
                ...
                {upload.status === 'uploading' && (
                  <div>
                    ...
                    <div style={{ marginTop: 12, height: 3, background: D.border, borderRadius: 2, overflow: 'hidden' }}>
                      <motion.div
                        animate={{ x: ['-100%', '100%'] }}
                        transition={{ repeat: Infinity, duration: 1.2, ease: 'linear' }}
                        style={{ height: '100%', width: '40%', background: '#8b5cf6', borderRadius: 2 }}
                      />
                    </div>
                  </div>
                )}
                ...
                {upload.status === 'success' && (
                  <div>
                    ...
                    <div style={{ color: D.green, fontSize: 13, fontWeight: 700, marginBottom: 4 }}>
                      {upload.fileName} indexed
                    </div>
                    <div style={{ color: D.sub, fontSize: 11 }}>
                      {upload.chunks} chunks stored · Agent 1 will use this on next run
                    </div>
                  </div>
                )}
                ...
                {upload.status === 'error' && (
                  <div>
                    <div style={{ fontSize: 22, marginBottom: 8 }}>⚠️</div>
                    <div style={{ color: D.red, fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
                      Upload failed
                    </div>
                    <div style={{ color: D.sub, fontSize: 11 }}>{upload.error}</div>
                    <div style={{ color: '#a78bfa', fontSize: 11, marginTop: 6 }}>Click to try again</div>
                  </div>
                )}
              </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {docs.map((doc, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{ fontSize: 14 }}>{FILE_ICONS[doc.file_type] || '📄'}</span>
                        <span style={{ flex: 1, color: D.text, fontSize: 12, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {doc.source_file}
                        </span>
                        <span style={{
                          background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.2)',
                          borderRadius: 10, padding: '1px 7px',
                          fontSize: 10, color: '#a78bfa', fontWeight: 600, flexShrink: 0,
                        }}>
                          {doc.chunks} chunks
                        </span>
                      </div>
              ...
              <div style={{
                ...
                <span style={{ color: D.dim, fontSize: 11, lineHeight: 1.6 }}>
                  Upload a product spec, press release, or internal report. Agent 1 queries these documents before drafting — your content will include accurate product details, real metrics, and internal positioning that the LLM alone cannot know.
                </span>
              </div>
            </div>
          </motion.div>
