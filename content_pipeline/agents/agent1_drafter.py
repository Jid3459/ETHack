...
def _revise_blog_sections(
    ...
    for section in outline.get("sections", []):
        ...
        section_violations = [
            ...
            if v.get("phrase", "").lower() in section_text_lower
        ]
        section_legal = [
            f for f in high_legal
            if f.get("claim", "").lower() in section_text_lower
        ]

        # Retrieve regulatory constraints fresh for this section
        section_constraints = _retriever.retrieve(section.get("key_point", heading), limit=3)
        ...
        combined_violations = list(section_violations) + [
            {
                ...
                "fix_suggestion": f.get("fix_suggestion", "Rephrase to remove unsubstantiated claim"),
            }
        ...
        messages = [
            ...
            HumanMessage(
                content=_SECTION_REVISION_PROMPT.format(
                    ...
                    violations_formatted=_format_violations_for_revision(combined_violations),
...
def _generate_short_form(
    ...
    if state["revision_count"] > 0:
        ...
        product_knowledge = _fetch_product_knowledge(state["brief"], state["company_id"])
        messages = [
            SystemMessage(content=_build_system_prompt(profile_fields)),
            HumanMessage(
